...
async function loadDailyAlmanac() {
    try {
        const res = await fetch('/api/daily-almanac');
...
async function showTestResult() {
    ...
    try {
        const res = await fetch('/api/test-fate', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ answers: state.testAnswers })
        });
        const data = await res.json();
        if (data.code === 0) {
...
async function drawTarot() {
    ...
    try {
        const res = await fetch('/api/tarot', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ spread: state.tarotSpread, question })
        });
        const data = await res.json();
        if (data.code === 0) {
...
async function loadZodiacFortune(sign, period) {
    ...
    try {
        const res = await fetch(`/api/zodiac/${sign}?period=${period}`);
        const data = await res.json();
        if (data.code === 0) {
...
async function submitZiwei() {
    ...
    try {
        const res = await fetch('/api/ziwei', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ birthYear: year, birthMonth: month, birthDay: day, birthHour: hour, gender })
        });
        const data = await res.json();
        if (data.code === 0) {
...
async function submitYearly() {
    ...
    try {
        const res = await fetch('/api/yearly-fortune', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ birthYear, zodiac })
        });
        const data = await res.json();
        if (data.code === 0) {
...
function showPayModal(defaultOption) {
    ...
    document.getElementById('pay-btn-start').disabled = false;
    ...
    try {
        const res = await fetch('/api/pay/create', {
            ...
            body: JSON.stringify({
                ...
                productType: state.selectedPayOption
            })
        });
        ...
        if (!res.ok) {
            ...
            showPayError(text || '服务器响应异常，请稍后重试');
        ...
        if (data.code !== 0) {
            showPayError(data.msg || '创建订单失败，请稍后重试');
            btn.textContent = '立即支付';
            btn.disabled = false;
            return;
        }
        ...
        confirmBtn.onclick = () => confirmPaymentDone(orderData.orderNo);
...
async function confirmPaymentDone(orderNo) {
    ...
    try {
        const res = await fetch('/api/pay/mock-success', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
