...
function getOrderByNo(orderNo) {
    return new Promise((resolve) => {
        const row = db.get('SELECT * FROM orders WHERE order_no = ?', [orderNo]);
        resolve(row);
    });
}
...
function updateOrderPaid(orderNo, payTime) {
    return new Promise((resolve) => {
        const count = db.update('orders', { order_no: orderNo }, { status: 1, pay_time: payTime });
        resolve(count);
    });
}
...
function checkUserVip(userId) {
    return new Promise((resolve) => {
        const row = db.get('SELECT is_vip FROM users WHERE id = ?', [userId]);
        resolve(row && row.is_vip === 1);
    });
}
...
function setUserVip(userId) {
    return new Promise((resolve) => {
        const count = db.update('users', { id: userId }, { is_vip: 1, vip_expire: 9999999999 });
        resolve(count);
    });
}
...
function saveDivinationRecord(userId, type, inputData, resultData, isFree) {
    return new Promise((resolve) => {
        const row = db.insert('divination_records', {
            user_id: userId,
            type: type,
...
function getDailyAlmanac(date) {
    return new Promise((resolve) => {
        const row = db.get('SELECT * FROM daily_almanac WHERE date = ?', [date]);
        if (row) {
            resolve({
                date: row.date,
                yi: row.yi,
            ...
            db.insert('daily_almanac', {
                date: almanac.date,
                yi: almanac.yi,
                ji: almanac.ji,
                lucky_color: almanac.luckyColor,
                lucky_color_value: almanac.luckyColorValue,
                blessing: almanac.blessing,
                fortune_score: almanac.fortuneScore
            });
            resolve(almanac);
        }
    });
