...
import { Compass, Sparkles, User, Calendar, Hash, ChevronRight, RotateCcw, Star, Crown } from 'lucide-react';
...
export default function Destiny() {
  ...
  const handleResult = (r: string) => {
    ...
    if (user?.memberType === 'single') {
      // 单次会员：检查是否已使用
      const used = localStorage.getItem('single_used');
      if (!used) {
        localStorage.setItem('single_used', 'true');
        setPaid(true);
        setResult(r);
        window.scrollTo({ top: 0, behavior: 'smooth' });
        return;
            ...
            <div className="liquid-glass rounded-3xl p-6 md:p-8">
              <div className="flex items-center justify-between mb-6">
                ...
                {user?.memberType === 'lifetime' && (
                  <span className="flex items-center gap-1 px-3 py-1 rounded-full bg-[rgba(239,182,255,0.15)] text-[#efb6ff] text-xs">
                    <Crown className="w-3 h-3" />终身会员
                  </span>
                )}
              </div>
